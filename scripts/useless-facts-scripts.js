/*
These scripts are in charge of loading a random useless fact every two and a half seconds
while the page is loading to amuse the visitor.
 */
...
// use to ensure new fact every 5 seconds
var factLifeTime = 5000;
// counter for milliseconds
var millisecondsCounter = 0;
// track facts shown so we don't display the same one twice
var uselessFactsShown = [];

function uselessFactsUntilEverythingLoaded(condition, callback, factTimeCounter) {
    if (!condition()) {
        // load a new useless fact every 5 seconds unless everything is loaded
        if (factTimeCounter % factLifeTime == 0) {
            generateUselessFact();
        }
        // check for document load every 100 milliseconds
        window.setTimeout(uselessFactsUntilEverythingLoaded.bind(null, condition, callback, millisecondsCounter += 100), 100);
    }
    if (condition()) {
...
uselessFactsUntilEverythingLoaded(function() {
    return window.stopUselessFacts;
}, function() {
    console.info('document loaded. stop facts');
    pageHasLoaded();
});
...
//     var navBar = document.getElementsByTagName('NAV').classList;
//     if (navBar != undefined) {
//         navBar.remove('hide');
//     }
...
function generateUselessFact() {
    var uselessFactNumber = Math.floor((Math.random() * 99) + 0);
    console.info('generating useless fact: ', uselessFactNumber);
    console.info('useless fact count: ', uselessFactsShown.length);
    if (uselessFactsShown.length >= 100) {
        // if we have shown all 100 facts something is probably wrong...
        console.error('We have shown all facts, there might be something wrong...');
        document.getElementById('useless-facts-heading').innerHTML = "There might be something wrong...";
        document.getElementById('factNumber').innerHTML = '???';
        document.getElementById('fact').innerHTML = 'Have you tried turning it off and on again?';
        window.stopUselessFacts = true;
    } else if (uselessFactNumber < 100 && uselessFactsShown.indexOf(uselessFactNumber.toString()) == -1) {
        // keep showing unique facts
        document.getElementById('factNumber').innerHTML = uselessFactNumber;
        document.getElementById('fact').innerHTML = uselessFacts[uselessFactNumber];
        uselessFactsShown.push(uselessFactNumber.toString());
    } else {
        // if keep showing facts and were going to show non unique, get another one
        console.info('this isn\'t a unique fact, better get another one.');
        generateUselessFact();
    }
}